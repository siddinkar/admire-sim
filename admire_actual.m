...
for t=1:length(tfspan)
    ...
    r_m(t) = l_m * (x0' * x0) / (l_m * (x0' * x0) + gamma * sqrt(x0' * x0) * sqrt(3) + sum);
    r_a(t) = sum + gamma * sqrt(x0' * x0) * sqrt(3);

    E_nominal_series(t) = E_n;
    E_rand_series(t) = E_rand;
    % E_sin_series(t) = E_sin;
    E_const_series(t) = E_const;
end
...
plot(tfspan, r_m, 'b')
hold on
% plot(tfspan, mm_rand, 'r')
plot(tfspan, mm_const, 'g')
xlabel('Range')
ylabel('$E_N / E_D$')
...
plot(tfspan, r_a, 'b')
hold on
% plot(tfspan, am_rand, 'r')
plot(tfspan, am_const, 'g')
xlabel('Range')
ylabel('$E_D - E_N$')
